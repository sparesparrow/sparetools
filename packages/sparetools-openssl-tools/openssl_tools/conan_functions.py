...
from .file_operations import find_executable_in_path, find_first_existing_file
...
def get_default_conan() -> Path:
    ...
    if getattr(sys, 'frozen', False):
        ...
        conan_exe_name = 'conan.exe' if os.name == 'nt' else 'conan'
        conan_exe = find_executable_in_path(conan_exe_name)
        if conan_exe is None:
            # Fallback: try common installation paths including python-env
            python_env_paths = [
                str(Path.cwd().parent.parent / 'python-env' / 'bin' / conan_exe_name),
                str(Path.home() / 'projects' / 'openssl-test' / 'python-env' / 'bin' / conan_exe_name),
            ]
            conan_paths = python_env_paths + [
                '/usr/local/bin/conan',
                '/usr/bin/conan',
                '/opt/conan/bin/conan',
            ...
            conan_exe = find_first_existing_file(conan_paths, conan_exe_name)
...
def get_all_packages_in_cache() -> list:
    ...
    # Use conan list to get all packages in cache (Conan 2.x format)
    rc, return_string = execute_command(f'{get_default_conan()} list "*"')
    if rc == 0:
        # Parse the text output to extract package references
        packages = []
        for line in return_string:
            line = line.strip()
            # Look for lines with package/version format (like "cpython-tool/3.12.7")
            if '/' in line and line.count('/') == 1 and not line.startswith('Found') and not line.startswith('Local') and not line.startswith('ERROR'):
