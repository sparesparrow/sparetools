...
def execute_command(command: Union[str, List[str]],
    ...
    # Ensure PATH includes python-env/bin if it exists (for development)
    python_env_bin = Path(__file__).parent.parent.parent.parent / 'python-env' / 'bin'
    if python_env_bin.exists() and str(python_env_bin) not in env.get('PATH', ''):
        env['PATH'] = f"{python_env_bin}:{env['PATH']}"
