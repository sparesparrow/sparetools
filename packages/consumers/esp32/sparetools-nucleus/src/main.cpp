...
void setup() {
  ...
  // Initialize SpareTools HAL for Sunton display
  sunton_display_handle_t* disp_handle = sunton_display_init(NULL);
  if (!disp_handle) {
    Serial.println("Failed to initialize Sunton display");
    return;
  }

  // Initialize touch controller
  if (!sunton_touch_init(NULL)) {
    Serial.println("Failed to initialize Sunton touch");
    return;
  }
 ...
 bool touched() {
    ...
    uint16_t x, y;
    bool pressed;
    if (sunton_touch_read(&x, &y, &pressed)) {
        return pressed && (x > 0 && x < 320 && y > 0 && y < 320);
    }
    return false;
}
