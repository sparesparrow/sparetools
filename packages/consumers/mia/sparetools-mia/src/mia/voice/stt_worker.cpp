...
// Forward declarations for whisper.h to avoid including it directly
extern "C" {
    ...
    int whisper_full(whisper_context_t* ctx, struct whisper_full_params params, const float* samples, int n_samples);
    int whisper_full_n_segments(whisper_context_t* ctx);
    const char* whisper_full_get_segment_text(whisper_context_t* ctx, int i);
}
