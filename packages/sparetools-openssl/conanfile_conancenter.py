...
class OpenSSLConan(ConanFile):
    """
    OpenSSL library package for ConanCenter.

    A robust, commercial-grade, and full-featured toolkit for the Transport Layer Security (TLS)
    and Secure Sockets Layer (SSL) protocols as well as a full-strength general purpose
    cryptographic library.

    This recipe supports multiple build methods: Perl Configure (default), CMake, and Autotools.
    """
    ...
    topics = ("openssl", "ssl", "tls", "encryption", "security", "cryptography")
    ...
    def _build_with_perl(self):
        ...
        if is_windows:
            ...
            build_cmd = "nmake"
            test_cmd = "nmake test" if not self.options.no_tests else None
        else:
            ...
            test_cmd = "make test" if not self.options.no_tests else None
