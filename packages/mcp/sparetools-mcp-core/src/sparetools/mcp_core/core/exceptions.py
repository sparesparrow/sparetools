"""Custom exceptions for SpareTools MCP Core.

This module defines custom exceptions used throughout the package for specific error cases.
...
class ErrorCode(Enum):
    ...
    # Device errors (E08x)
    DEVICE_NOT_FOUND = "E080"
    DEVICE_CONNECTION_FAILED = "E081"
    DEVICE_COMMUNICATION_ERROR = "E082"
    DEVICE_DETECTION_FAILED = "E083"

    # Deployment errors (E09x)
    DEPLOYMENT_FAILED = "E090"
    PROFILE_NOT_FOUND = "E091"
    PROFILE_INVALID = "E092"
    BUILD_FAILED = "E093"
    FLASH_FAILED = "E094"


class MCPException(Exception):
    """Base exception class for MCP Core with enhanced error tracking.
...
class DeviceError(MCPException):
    ...
    def __init__(
        ...
        code: ErrorCode = ErrorCode.DEVICE_NOT_FOUND,
        cause: Optional[Exception] = None,
    ) -> None:
        """Initialize the exception.
        ...
        Args:
            message: Error message
            device_port: Optional port of the device that caused the error
            device_type: Optional type of the device
            code: Error code (default: DEVICE_NOT_FOUND)
            cause: Optional underlying exception
        """
        details = {}
        if device_port:
...
class DeploymentError(MCPException):
    ...
    def __init__(
        ...
        Args:
            ...
            code: Error code (default: DEPLOYMENT_FAILED)
            cause: Optional underlying exception
        """
        details = {}
        if target:
